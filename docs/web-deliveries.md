...
The embedded delivery application is a React root composed from the repository-owned shadcn [`@courier/ui`](../web/ui) package. It uses the pixel Relay mark in product chrome and only the 384×384 delivery-role sprite as optional decoration. That sprite keeps the canonical mascot's anatomy and changes only the parcel-holding pose. Authentication, route state, and the manifest render directly on the dithered document canvas without card fills, outer radii, or nested tonal blocks; pixel inputs, alerts, status, focus, manifest/file icons, and row boundaries retain explicit native semantics. Metadata, upload, and download behavior remain backed exclusively by the existing versioned API.
