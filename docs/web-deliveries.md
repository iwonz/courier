...
The embedded delivery application is a React root composed from the repository-owned shadcn [`@courier/ui`](../web/ui) package. It uses the square Relay mark in product chrome and the full mascot only as decoration. Authentication, route state, and the manifest render directly on the document canvas without card fills, outer radii, or nested tonal blocks; inputs, alerts, status, focus, and manifest row boundaries retain explicit semantics. Metadata, upload, and download behavior remain native controls backed exclusively by the versioned API.
