...
Every section uses natural content height and compact adjacent spacing: there are no viewport-height targets, full-screen slides, snap points, rounded boards, detached card islands, contract-count strips, or boundary rules between the header and sections. The headline, Relay, and command workspace sit directly on the canvas without a light card fill. Installation uses horizontally scrollable channel tabs with stable, immediate active and hover states plus one unframed command area. A direct-binaries link with a functional download icon remains beside the tabs, outside their scroller, including on mobile. The nested `#cli` workspace uses responsive command selection and options without an outer table frame, command-list label, header rules, row rules, inter-column dividers, or a readout rule. Neither command readout repeats a visible `Command` label. The transparent `h-16` header is a static normal-flow row that scrolls away. It vertically centers the square Relay mark and `COURIER CLI` wordmark on the left, with GitHub plus preference controls on the right, and has no internal navigation or compensating main padding.
