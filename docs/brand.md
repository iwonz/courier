...
Across the mark, route, delivery, and administration roles, Relay keeps the same silhouette, physiology, body-to-head proportions, eye and beak geometry, plumage, earpiece, satchel, and palette. Only pose, role equipment, and a carried or attached object may change. Every new role starts as an identity-preserving derivative of the canonical neutral sprite; an independent mascot redraw is not an acceptable variation. Relay never gains armor, a helmet, visor, glowing face panel, exoskeleton, metallic chest plate, police or military equipment, photorealism, smooth 3D shading, scenery, text, or a watermark.

Use [`courier-relay-pixel-mark-v2.webp`](../web/ui/assets/courier-relay-pixel-mark-v2.webp) in headers and favicons. Use neutral-v1 as the canonical identity reference and route/delivery/admin-v2 only in their declared roles. The [v2 QA contact sheet](assets/courier-relay-pixel-v2-contact-sheet.png) compares the complete family on light, dark, and checkerboard backgrounds. Product meaning remains complete when artwork is unavailable; a sprite never replaces a label, status, or security warning. Every sprite is transparent, square, lossless WebP rendered with pixelated sampling.
...
The canonical neutral-v1 Relay was generated first with the built-in ImageGen workflow. Mark, route, delivery, and admin were regenerated as identity-preserving v2 derivatives with neutral-v1 as their explicit identity authority and the old role only as a pose/object reference. Accepted PNG sources were reviewed, nearest-neighbor resized, reduced to a shared hard 32-color raster treatment, and encoded as lossless transparent WebP. Source outputs are retained under `output/imagegen`; superseded role-v1 delivery copies are not shipped.

[`provenance.json`](../web/ui/assets/provenance.json) records identity invariants, allowed variations, exact prompts, ImageGen lineage, consumers, role, dimensions, byte count, SHA-256, contact sheet, font revision, and license. The asset gate verifies exactly five declared Relay sprites, alpha transparency, square dimensions, the 24/64/64/48/48 KiB per-role limits, the 248 KiB combined limit, the pinned local font and OFL notice, and the absence of undeclared or legacy role assets.
