...
`,ye=class extends q{constructor(...e){super(...e),this.disabled=!1,this.variant=`secondary`}static{this.properties={disabled:{type:Boolean,reflect:!0},variant:{type:String,reflect:!0}}}static{this.styles=[J,o`
  ...
  `}render(){let e=Ve(this.name);return I`<svg
    ...
    input, select { min-height: 2.75rem; padding: 0 .5rem; }
  ...
  `}connectedCallback(){super.connectedCallback(),this.theme=Le(),this.unsubscribe=qe(e=>{this.snapshot=e,this.failed=!1}),this.refresh()}disconnectedCallback(){this.unsubscribe?.(),this.theme?.destroy(),super.disconnectedCallback()}async refresh(){this.failed=!1,this.conflict=!1;try{this.snapshot=await We()}catch{this.failed=!0}}setLocale(e){this.locale=e.detail}async stop(e,t){try{await Ke(e,t),await this.refresh()}catch{this.failed=!0}}async save(e,t){e.preventDefault();let n=new FormData(e.currentTarget),r={...t.policy,version:t.policy.version+1,auth:String(n.get(`auth`)),authAttempts:Number(n.get(`attempts`)),authFailAction:String(n.get(`failAction`)),noUi:n.get(`noUi`)===`on`};this.failed=!1,this.conflict=!1;try{await Ge(t,r),await this.refresh()}catch(e){this.conflict=e instanceof Error&&e.name===`ConflictError`,this.failed=!this.conflict}}t(e){return Xe(this.locale,e)}delivery(e){return I`
      <article>
    ...
    `}render(){let e=this.snapshot?.servers??[];return I`
      <main>
        <header>
          <h1>${this.t(`title`)}</h1>
          <nav><courier-button @click=${this.refresh}>${this.t(`refresh`)}</courier-button><courier-theme-selector></courier-theme-selector><courier-locale-selector @courier-locale=${this.setLocale}></courier-locale-selector></nav>
        </header>
    ...
    `}};customElements.get(`courier-admin-app`)||customElements.define(`courier-admin-app`,Ze);