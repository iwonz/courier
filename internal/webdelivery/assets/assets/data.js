...
`,we=class extends U{constructor(...e){super(...e),this.disabled=!1,this.type=`button`,this.variant=`secondary`}static{this.properties={disabled:{type:Boolean,reflect:!0},type:{type:String,reflect:!0},variant:{type:String,reflect:!0}}}static{this.styles=[Se,o`
  ...
  `}activate(e){let t=e.currentTarget.dataset.value??``;t&&t!==this.value&&(this.value=t,this.dispatchEvent(new CustomEvent(`courier-segment-change`,{detail:t,bubbles:!0,composed:!0})))}move(e){let t=[...this.renderRoot.querySelectorAll(`button`)],n=t.indexOf(e.currentTarget),r=at(e.key,n,t.length);if(r===void 0)return;e.preventDefault();let i=t[r];i.focus(),i.click()}render(){return P`<fieldset>
    ...
    </fieldset>`}};function X(e){return Math.min(100,Math.max(0,e))}function st(e,t,n){return e.width<=0||e.height<=0?{x:50,y:50}:{x:X((t-e.left)/e.width*100),y:X((n-e.top)/e.height*100)}}var ct=class extends U{constructor(...e){super(...e),this.eager=!1,this.mobileSource=``,this.source=``,this.frame=0}static{this.properties={eager:{type:Boolean},mobileSource:{type:String,attribute:`mobile-source`},source:{type:String}}}static{this.styles=o`
  ...
  `}disconnectedCallback(){this.frame&&globalThis.cancelAnimationFrame(this.frame),this.frame=0,super.disconnectedCallback()}tracksPointer(){return globalThis.matchMedia?.(`(hover: hover) and (pointer: fine)`).matches===!0&&!globalThis.matchMedia?.(`(prefers-reduced-motion: reduce)`).matches}move(e){this.tracksPointer()&&(this.pending=st(this.getBoundingClientRect(),e.clientX,e.clientY),!this.frame&&(this.frame=globalThis.requestAnimationFrame(()=>{this.frame=0;let e=this.pending;e&&(this.style.setProperty(`--scene-pointer-x`,`${e.x}%`),this.style.setProperty(`--scene-pointer-y`,`${e.y}%`))})))}leave(){this.pending=void 0,this.frame&&globalThis.cancelAnimationFrame(this.frame),this.frame=0,this.style.removeProperty(`--scene-pointer-x`),this.style.removeProperty(`--scene-pointer-y`)}render(){return P`<courier-mascot class="base" ?eager=${this.eager} alt="" .source=${this.source} .mobileSource=${this.mobileSource} @pointermove=${this.move} @pointerleave=${this.leave}></courier-mascot><courier-mascot class="refracted" aria-hidden="true" alt="" .source=${this.source} .mobileSource=${this.mobileSource}></courier-mascot><span class="glow" aria-hidden="true"></span><span class="veil" aria-hidden="true"></span>`}},lt=[`system`,`light`,`dark`],ut=`courier.theme`;function dt(e){return lt.includes(e)?e:`system`}function ft(e,t){return e===`system`?t?.matches?`dark`:`light`:e}function pt(e){if(!e)return`system`;try{return dt(e.getItem(ut))}catch{return`system`}}function mt(e,t){if(e)try{e.setItem(ut,t)}catch{}}var ht=class{constructor(e,t,n,r){this.root=e,this.storage=t,this.media=n,this.onSystemChange=()=>this.apply(),this.preference=r??pt(t),this.media?.addEventListener(`change`,this.onSystemChange),this.apply()}set(e){this.preference=e,mt(this.storage,e),this.apply()}destroy(){this.media?.removeEventListener(`change`,this.onSystemChange)}apply(){this.root.dataset.courierTheme=ft(this.preference,this.media),this.root.dataset.courierThemePreference=this.preference}};function gt(){let e;try{e=globalThis.localStorage}catch{e=void 0}let t=globalThis.matchMedia?.(`(prefers-color-scheme: dark)`);return new ht(document.documentElement,e,t)}var _t=class extends U{constructor(...e){super(...e),this.preference=`system`,this.locale=`en`}static{this.properties={preference:{type:String},locale:{type:String}}}connectedCallback(){super.connectedCallback(),this.state=gt(),this.preference=this.state.preference}disconnectedCallback(){this.state?.destroy(),super.disconnectedCallback()}change(e){let t=dt(e.detail);this.preference=t,this.state?.set(t),this.dispatchEvent(new CustomEvent(`courier-theme-change`,{detail:t,bubbles:!0,composed:!0}))}render(){return P`<courier-segmented-control
      icon-only
  ...
  `}render(){let e=bt(this.name);return P`<svg
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      stroke-width="1.7"
  ...
  `]}connectedCallback(){super.connectedCallback(),this.theme=gt(),this.refresh()}disconnectedCallback(){this.theme?.destroy(),super.disconnectedCallback()}async refresh(e=this.metadata?.path??``){this.failed=!1;try{this.metadata=await wt(e)}catch{this.failed=!0,this.metadata=void 0}}async openDirectory(e,t){e.preventDefault(),await this.refresh(t)}async signIn(e){e.preventDefault();let t=e.currentTarget,n=new FormData(t).get(`password`)?.toString()??``;try{this.csrf=(await Tt(n)).csrf,t.reset(),await this.refresh()}catch{this.failed=!0}}async sendFile(e){let t=e.currentTarget,n=t.files?.item(0);if(n)try{await Et(n,this.csrf),t.value=``,await this.refresh()}catch{this.failed=!0}}setLocale(e){this.locale=e.detail}t(e){return At(this.locale,e)}entry(e){let t=Dt(this.metadata.path,e.name);return e.type===`directory`?P`<li><courier-icon name="folder"></courier-icon><button class="link entry-name" @click=${e=>this.openDirectory(e,t)}>${e.name}</button><span class="size">${e.size} ${this.t(`itemSize`)}</span><a href=${$(t,!0)}>${this.t(`downloadArchive`)}</a></li>`:P`<li><courier-icon name="parcel"></courier-icon><span class="entry-name">${e.name}</span><span class="size">${e.size} ${this.t(`itemSize`)}</span><a href=${$(t)}>${this.t(`download`)}</a></li>`}render(){let e=this.metadata?.entries??[];return P`
      <main>
        <header>
        ...
        <div class="workspace">
          ...
          ${this.failed?P`
            <div class="access">
              ...
              <div class="access-art"><courier-mascot alt="" .source=${Ct}></courier-mascot></div>
            </div>
          `:I}
          ${this.metadata?P`
            ...
            <section class="delivery-panel">
              ...
              `:P`
                ${this.metadata.type===`file`?P`<div class="toolbar"><courier-icon name="download"></courier-icon><a href=${$(this.metadata.path)}>${this.t(`download`)}</a></div>`:P`
                  <div class="toolbar"><a href=${$(this.metadata.path,!0)}>${this.t(`downloadAll`)}</a>${this.metadata.path?P`<button class="link" @click=${e=>this.openDirectory(e,Ot(this.metadata.path))}>${this.t(`up`)}</button>`:I}</div>
    ...
    `}};customElements.get(`courier-data-app`)||customElements.define(`courier-data-app`,jt);