...
Courier SHALL create a gzip-compressed POSIX tar archive named `<source-name>.tar.gz` using its built-in implementation, without requiring an external archiver.

#### Scenario: Directory archive

- **WHEN** archive mode is requested for source directory `photos`
- **THEN** a private temporary `photos.tar.gz` contains the source root and its supported descendants
