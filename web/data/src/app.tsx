...
function DataContent(): React.JSX.Element {
  ...
  const entry = (item: Entry): React.JSX.Element => {
    ...
    return <li key={`${item.type}:${item.name}`} className="grid min-w-0 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3">
      <span className="grid size-9 place-items-center text-primary">{item.type === "directory" ? <Folder className="size-4" /> : <PackageOpen className="size-4" />}</span>
  ...
  return <div className="min-h-screen px-4 pb-16 sm:px-6 lg:px-8">
    ...
    <main className="mx-auto grid w-full max-w-6xl gap-6 pt-8 sm:pt-14">
      ...
      {failed && !metadata ? <Card data-courier-auth-region className="mx-auto w-full max-w-xl">
        <CardHeader><div className="mb-2 text-primary"><ShieldCheck className="size-8" /></div><CardTitle>{t("accessTitle")}</CardTitle><CardDescription>{t("accessHelp")}</CardDescription></CardHeader>
      ...
      {metadata ? <>
        ...
        <Card data-courier-manifest><CardHeader className="flex-row items-center justify-between gap-4"><div><CardTitle>{metadata.name}</CardTitle><CardDescription>{t("manifest")}</CardDescription></div><Badge variant="success"><ShieldCheck className="size-3.5" />{t("ready")}</Badge></CardHeader>
