...
function AdminContent(): React.JSX.Element {
  ...
  const serverNavigator = (server: Server): React.JSX.Element => <section key={server.id} className="grid gap-3 border-b border-border p-4 last:border-b-0">
    <div className="grid gap-2"><div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0 flex-1"><p className="truncate font-mono text-xs font-semibold">{server.id}</p><p className="truncate font-mono text-xs text-muted-foreground">{server.bind}</p></div><Badge className="shrink-0" variant={server.status === "live" ? "success" : "destructive"}>{t(server.status)}</Badge></div><Button type="button" size="sm" variant="outline" onClick={() => void stop("servers", server.id)}><PixelIcon name="stop" />{t("stopServer")}</Button></div>
  ...
  return <div className="min-h-screen pb-16">
    <header className="border-b border-border bg-background"><div className="mx-auto flex h-16 w-full max-w-7xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8"><Brand /><div className="flex items-center gap-2"><Button type="button" variant="ghost" size="icon" aria-label={t("refresh")} onClick={() => void refresh()}><PixelIcon name="refresh" /></Button><ThemeSelector /><LocaleSelector /></div></div></header>
    <main className="mx-auto grid w-full max-w-7xl gap-0 px-4 pt-8 sm:px-6 sm:pt-12 lg:px-8">
      <div className="grid items-center border-y border-border bg-card lg:grid-cols-[minmax(0,1fr)_18rem]"><div className="grid gap-4 p-5 sm:p-8"><Badge variant="secondary" className="w-fit font-mono uppercase tracking-[.08em]">{t("eyebrow")}</Badge><h1 className="text-5xl font-bold tracking-[-.035em] sm:text-7xl">{t("title")}</h1><p className="max-w-2xl text-sm leading-relaxed text-muted-foreground">{t("intro")}</p></div><RelaySprite role="admin" className="hidden w-full max-w-64 justify-self-end p-4 lg:block" /></div>

      <div data-courier-metrics className="grid border-b border-border bg-card divide-y divide-border sm:grid-cols-3 sm:divide-x sm:divide-y-0">
      ...
      {failed ? <Alert variant="destructive" className="mt-6"><AlertTitle>{t("unreachable")}</AlertTitle><AlertDescription className="flex flex-wrap items-center justify-between gap-3"><span>{t("failed")}</span><Button type="button" variant="outline" size="sm" onClick={() => void refresh()}>{t("retry")}</Button></AlertDescription></Alert> : null}
      {conflict ? <Alert variant="warning" className="mt-6"><AlertTitle>{t("policy")}</AlertTitle><AlertDescription className="flex flex-wrap items-center justify-between gap-3"><span>{t("conflict")}</span><Button type="button" variant="outline" size="sm" onClick={() => void refresh()}>{t("refresh")}</Button></AlertDescription></Alert> : null}

      {snapshot ? servers.length ? <Card data-courier-admin-workspace className="mt-8 border-y border-border bg-card"><div className="grid min-h-[34rem] lg:grid-cols-[20rem_minmax(0,1fr)]"><ScrollArea className="h-[34rem] border-b border-border lg:border-b-0 lg:border-r">{servers.map(serverNavigator)}</ScrollArea><div className="min-w-0">{selected ? <article className="grid gap-5 p-5 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold uppercase tracking-[.08em] text-muted-foreground">{selected.route}</p><p className="mt-1 break-all font-mono text-sm font-bold">{selected.id}</p></div><Button type="button" variant="destructive" size="sm" onClick={() => void stop("deliveries", selected.id)}><PixelIcon name="stop" />{t("stopDelivery")}</Button></div><RouteDisplay className="border-y border-border" source={selected.source || t("unavailable")} destination={selected.destination || t("unavailable")} /><div className="grid gap-4 border-b border-border pb-5 sm:grid-cols-3"><div><span className="text-xs text-muted-foreground">{t("source")}</span><p className="break-all font-mono text-xs font-semibold">{selected.source || t("unavailable")}</p></div><div><span className="text-xs text-muted-foreground">{t("destination")}</span><p className="break-all font-mono text-xs font-semibold">{selected.destination || t("unavailable")}</p></div><div><span className="text-xs text-muted-foreground">{t("transferred")}</span><p className="font-mono text-xs font-semibold text-warning">{selected.counters.confirmed}</p></div></div><form className="grid gap-4 sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto_auto] xl:items-end" onSubmit={(event) => void save(event, selected)}>
      ...
      </form></article> : <div className="grid min-h-[34rem] place-items-center p-6 text-sm text-muted-foreground">{t("empty")}</div>}</div></div></Card> : <Card className="mt-8 border-y border-border bg-card"><CardHeader><CardTitle>{t("live")}</CardTitle><CardDescription>{t("empty")}</CardDescription></CardHeader></Card> : !failed ? <Card className="mt-8 border-y border-border bg-card"><CardContent className="flex min-h-40 items-center justify-center gap-2 text-sm text-muted-foreground"><PixelIcon name="activity" className="size-4" />{t("loading")}</CardContent></Card> : null}
    </main>
  </div>;
}
