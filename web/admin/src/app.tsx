...
import { Activity, Database, RefreshCw, Save, ServerCog, Square, Workflow } from "lucide-react";
...
function AdminContent(): React.JSX.Element {
  ...
  const serverNavigator = (server: Server): React.JSX.Element => <section key={server.id} className="grid gap-2 py-3">
    <div className="grid gap-2 rounded-2xl bg-muted/40 p-3"><div className="flex items-start justify-between gap-2"><div className="min-w-0"><p className="truncate font-mono text-xs font-semibold">{server.id}</p><p className="truncate font-mono text-xs text-muted-foreground">{server.bind}</p></div><Badge variant={server.status === "live" ? "success" : "destructive"}>{t(server.status)}</Badge></div><Button type="button" size="sm" variant="outline" onClick={() => void stop("servers", server.id)}><Square />{t("stopServer")}</Button></div>
  ...
  return <div className="min-h-screen px-4 pb-16 sm:px-6 lg:px-8">
    <header className="mx-auto flex min-h-20 w-full max-w-7xl items-center justify-between gap-4"><Brand /><nav className="flex items-center gap-2"><Button type="button" variant="outline" size="icon" aria-label={t("refresh")} onClick={() => void refresh()}><RefreshCw /></Button><ThemeSelector /><LocaleSelector /></nav></header>
    <main className="mx-auto grid w-full max-w-7xl gap-6 pt-8 sm:pt-12">
      <div className="grid items-end gap-8 lg:grid-cols-[minmax(0,1fr)_18rem]"><div className="grid gap-4"><Badge variant="outline" className="font-mono">{t("eyebrow")}</Badge><h1 className="text-5xl font-black tracking-[-.06em] sm:text-7xl">{t("title")}</h1><p className="max-w-2xl text-base leading-relaxed text-muted-foreground">{t("intro")}</p></div><Mascot className="hidden w-full max-w-72 justify-self-end opacity-90 drop-shadow-[0_24px_30px_rgba(20,52,120,.24)] lg:block" /></div>

      <div className="grid gap-3 sm:grid-cols-3">
        <Card><CardContent className="flex items-center gap-4 p-5"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><ServerCog /></span><div><strong className="font-mono text-2xl">{servers.length}</strong><p className="text-xs text-muted-foreground">{t("serversMetric")}</p></div></CardContent></Card>
        <Card><CardContent className="flex items-center gap-4 p-5"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><Workflow /></span><div><strong className="font-mono text-2xl">{deliveries.length}</strong><p className="text-xs text-muted-foreground">{t("deliveriesMetric")}</p></div></CardContent></Card>
        <Card><CardContent className="flex items-center gap-4 p-5"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><Database /></span><div><strong className="font-mono text-2xl">{confirmed}</strong><p className="text-xs text-muted-foreground">{t("confirmedMetric")}</p></div></CardContent></Card>
      </div>
      ...
      {snapshot ? servers.length ? <Card className="overflow-hidden"><div className="grid min-h-[34rem] lg:grid-cols-[20rem_minmax(0,1fr)]"><ScrollArea className="h-[34rem] border-b border-border/70 p-3 lg:border-b-0 lg:border-r">{servers.map(serverNavigator)}</ScrollArea><div className="min-w-0">{selected ? <article className="grid gap-5 p-5 sm:p-7"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs font-semibold text-muted-foreground">{selected.route}</p><p className="mt-1 break-all font-mono text-sm font-bold">{selected.id}</p></div><Button type="button" variant="destructive" size="sm" onClick={() => void stop("deliveries", selected.id)}><Square />{t("stopDelivery")}</Button></div><RouteDisplay source={selected.source || t("unavailable")} destination={selected.destination || t("unavailable")} /><div className="grid gap-3 rounded-2xl border border-border/70 bg-muted/30 p-4 sm:grid-cols-3"><div><span className="text-xs text-muted-foreground">{t("source")}</span><p className="break-all font-mono text-xs font-semibold">{selected.source || t("unavailable")}</p></div><div><span className="text-xs text-muted-foreground">{t("destination")}</span><p className="break-all font-mono text-xs font-semibold">{selected.destination || t("unavailable")}</p></div><div><span className="text-xs text-muted-foreground">{t("transferred")}</span><p className="font-mono text-xs font-semibold">{selected.counters.confirmed}</p></div></div><Separator /><form className="grid gap-4 sm:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto_auto] xl:items-end" onSubmit={(event) => void save(event, selected)}>
