...
function AdminContent(): React.JSX.Element {
  ...
  return <div className="min-h-screen px-4 pb-16 sm:px-6 lg:px-8">
    <header className="mx-auto flex min-h-20 w-full max-w-7xl items-center justify-between gap-4"><Brand /><nav className="flex items-center gap-2"><Button type="button" variant="ghost" className="bg-muted/45" size="icon" aria-label={t("refresh")} onClick={() => void refresh()}><RefreshCw /></Button><ThemeSelector /><LocaleSelector /></nav></header>
    <main className="mx-auto grid w-full max-w-7xl gap-6 pt-8 sm:pt-12">
      <div className="grid items-end gap-8 lg:grid-cols-[minmax(0,1fr)_18rem]"><div className="grid gap-4"><Badge variant="secondary" className="w-fit font-mono">{t("eyebrow")}</Badge><h1 className="text-5xl font-black tracking-[-.06em] sm:text-7xl">{t("title")}</h1><p className="max-w-2xl text-base leading-relaxed text-muted-foreground">{t("intro")}</p></div><Mascot className="hidden w-full max-w-72 justify-self-end opacity-90 drop-shadow-[0_24px_30px_rgba(20,52,120,.24)] lg:block" /></div>

      <div data-courier-metrics className="grid overflow-hidden rounded-3xl bg-card/46 divide-y divide-border/35 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
        <div className="flex items-center gap-4 p-5"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><ServerCog /></span><div><strong className="font-mono text-2xl">{servers.length}</strong><p className="text-xs text-muted-foreground">{t("serversMetric")}</p></div></div>
        <div className="flex items-center gap-4 p-5"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><Workflow /></span><div><strong className="font-mono text-2xl">{deliveries.length}</strong><p className="text-xs text-muted-foreground">{t("deliveriesMetric")}</p></div></div>
        <div className="flex items-center gap-4 p-5"><span className="grid size-11 place-items-center rounded-2xl bg-primary/10 text-primary"><Database /></span><div><strong className="font-mono text-2xl">{confirmed}</strong><p className="text-xs text-muted-foreground">{t("confirmedMetric")}</p></div></div>
      </div>
