...
it("renders generated install, command, route, option, and documentation sections", async () => {
  ...
  expect(text).toContain("Move files safely");
  ...
  expect(element.shadowRoot?.querySelectorAll("section").length).toBe(6);

  element.setLocale(new CustomEvent("courier-locale", { detail: "ru" }));
  await element.updateComplete;
  expect(element.shadowRoot?.textContent).toContain("Безопасный перенос файлов");
  element.remove();
});
