...
export const landingStyles = css`
  ...
  * { box-sizing: border-box; }
  ...
  .brand-link, nav a, .download-channel { text-decoration: none; }
  ...
  .header-actions { --courier-control-frame-border: rgb(243 244 233 / 0.24); --courier-control-frame-color: var(--courier-paper-50); --courier-control-frame-surface: rgb(12 15 12 / 0.18); --courier-control-frame-hover-border: var(--courier-signal); --courier-control-frame-hover-color: var(--courier-signal); --courier-control-frame-hover-surface: rgb(12 15 12 / 0.32); margin-left: auto; gap: 0.5rem; }
  .preferences { gap: 0.35rem; }

  main { position: relative; background: linear-gradient(180deg, #d7d9ce 0%, #30372f 31%, #252b24 68%, #171a16 100%); }
  .slide { position: relative; min-height: max(42rem, 78svh); overflow: hidden; padding-top: calc(var(--masthead-height) + clamp(1rem, 3.2vw, 3rem)); padding-bottom: clamp(2.5rem, 7vh, 6rem); scroll-margin-top: var(--masthead-height); isolation: isolate; }
  .slide:not(.hero-slide) { content-visibility: auto; contain-intrinsic-size: auto 50rem; }
  .slide#cli { min-height: max(52rem, 88svh); contain-intrinsic-size: auto 60rem; }
  courier-scene { z-index: 0; top: -4rem; bottom: -4rem; -webkit-mask-image: linear-gradient(180deg, transparent 0, #000 4rem, #000 calc(100% - 4rem), transparent 100%); mask-image: linear-gradient(180deg, transparent 0, #000 4rem, #000 calc(100% - 4rem), transparent 100%); }
  .hero-slide > courier-scene { top: 0; -webkit-mask-image: linear-gradient(180deg, #000 0, #000 calc(100% - 4rem), transparent 100%); mask-image: linear-gradient(180deg, #000 0, #000 calc(100% - 4rem), transparent 100%); }
  #cli > courier-scene { bottom: 0; -webkit-mask-image: linear-gradient(180deg, transparent 0, #000 4rem, #000 100%); mask-image: linear-gradient(180deg, transparent 0, #000 4rem, #000 100%); }
  ...
  .slide-shell { position: relative; z-index: 3; display: grid; min-height: calc(max(42rem, 78svh) - var(--masthead-height) - clamp(3.5rem, 10vh, 9rem)); grid-template-rows: auto auto; align-content: center; gap: clamp(1rem, 3vh, 2rem); }
  #cli .slide-shell { min-height: calc(max(52rem, 88svh) - var(--masthead-height) - clamp(3.5rem, 10vh, 9rem)); }
  ...
  .hero-slide { min-height: 100svh; padding: 0; content-visibility: visible; }
  .hero-slide::after { background: none; }
  .hero { position: relative; z-index: 4; display: grid; min-height: 100svh; align-items: center; pointer-events: none; }
  .hero-copy { display: grid; width: min(62rem, 72%); color: var(--courier-graphite-950); }
  ...
  .route-explorer, .install-board, .cli-workspace { width: 100%; min-width: 0; min-height: 0; }
  .route-interface { display: grid; min-height: 0; grid-template-rows: auto auto; gap: clamp(0.75rem, 2vh, 1.25rem); }
  ...
  .route-readout { height: 12rem; min-height: 0; }
  ...
  .install-interface { display: grid; min-height: 0; grid-template-rows: auto auto auto; gap: clamp(0.75rem, 2vh, 1.25rem); }
  ...
  .install-readout { height: 12rem; min-height: 0; }
  ...
  .cli-workspace { display: grid; height: 34rem; min-height: 34rem; grid-template-columns: minmax(0, 1.22fr) minmax(20rem, 0.78fr); gap: 0.75rem; }
  ...
  @media (max-width: 44rem) {
    ...
    .header-actions { --courier-control-frame-size: 2.2rem; }
    .slide { min-height: 42rem; padding-top: calc(var(--masthead-height) + 1.2rem); padding-bottom: 3rem; }
    .slide#cli { min-height: 52rem; }
    .slide-shell, #cli .slide-shell { min-height: 0; align-content: start; gap: 0.75rem; }
    ...
    .route-interface { grid-template-rows: 8.6rem auto; gap: 0.6rem; }
    ...
    .route-readout, .install-readout, .cli-readout { height: 13.5rem; }
    .flag { padding: 0.16rem 0.26rem; font-size: 0.5rem; }
    .install-interface { gap: 0.6rem; }
    ...
    .cli-workspace { height: 54rem; min-height: 54rem; grid-template-columns: minmax(0, 1fr); grid-template-rows: minmax(27rem, 1fr) auto; gap: 0.6rem; }
  ...
  @media (max-height: 44rem) and (min-width: 44.01rem) {
    ...
    .slide { padding-top: calc(var(--masthead-height) + 1rem); padding-bottom: 2rem; }
