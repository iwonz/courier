...
export const landingStyles = css`
  :host { --masthead-height: 4.6rem; display: block; min-width: 20rem; color: var(--courier-color-text); background: var(--courier-color-canvas); font-family: var(--courier-font-sans); }
  .shell { width: min(100% - clamp(1.25rem, 5vw, 5rem), 94rem); margin-inline: auto; }
  .masthead-wrap { position: fixed; z-index: 30; top: 0; right: 0; left: 0; padding-top: env(safe-area-inset-top); background: linear-gradient(180deg, color-mix(in srgb, var(--courier-color-canvas) 88%, transparent) 0 42%, transparent); backdrop-filter: blur(10px); -webkit-mask-image: linear-gradient(#000 0 72%, transparent); mask-image: linear-gradient(#000 0 72%, transparent); }
  ...
  main { overflow: clip; }
  .stage { position: relative; min-height: max(42rem, 78svh); padding: calc(var(--masthead-height) + 3rem) 0 5rem; scroll-margin-top: var(--masthead-height); content-visibility: auto; contain-intrinsic-size: auto 54rem; isolation: isolate; }
  .stage::after { content: ""; position: absolute; z-index: 1; inset: 0; background: linear-gradient(90deg, color-mix(in srgb, var(--courier-color-canvas) 74%, transparent), transparent 48% 74%, color-mix(in srgb, var(--courier-color-canvas) 18%, transparent)); pointer-events: none; }
  .stage > courier-scene { z-index: 0; top: -3rem; bottom: -3rem; -webkit-mask-image: linear-gradient(180deg, transparent, #000 3rem, #000 calc(100% - 3rem), transparent); mask-image: linear-gradient(180deg, transparent, #000 3rem, #000 calc(100% - 3rem), transparent); }
  .route-stage { min-height: 100svh; padding-bottom: 3rem; content-visibility: visible; }
  .route-stage > courier-scene { top: 0; -webkit-mask-image: linear-gradient(#000 0 calc(100% - 3rem), transparent); mask-image: linear-gradient(#000 0 calc(100% - 3rem), transparent); }
  #cli { min-height: max(52rem, 88svh); }
  #cli > courier-scene { bottom: 0; -webkit-mask-image: linear-gradient(transparent, #000 3rem 100%); mask-image: linear-gradient(transparent, #000 3rem 100%); }
  .stage-shell { position: relative; z-index: 2; display: grid; min-height: 0; align-content: start; gap: clamp(1.5rem, 4vh, 3rem); }
  h1, h2, p { margin: 0; }
  h1 { max-width: 11ch; font-family: var(--courier-font-display); font-size: clamp(4.2rem, 10vw, 9rem); font-weight: 850; letter-spacing: -0.085em; line-height: 0.82; text-wrap: balance; }
  h2 { font-family: var(--courier-font-display); font-size: clamp(2.6rem, 6vw, 5.4rem); font-weight: 820; letter-spacing: -0.075em; line-height: 0.9; }
  ...
  .route-path { stroke: color-mix(in srgb, var(--courier-color-text) 34%, transparent); stroke-width: 1; }
  ...
  .endpoint { appearance: none; display: inline-grid; min-width: 0; min-height: 2.35rem; grid-template-columns: 1.35rem minmax(0, 1fr); align-items: center; gap: 0.38rem; padding: 0.25rem 0.55rem 0.25rem 0.28rem; border: 1px solid color-mix(in srgb, var(--courier-color-border) 70%, transparent); border-radius: 999px; color: var(--courier-color-text); background: color-mix(in srgb, var(--courier-color-canvas) 60%, transparent); text-align: left; cursor: pointer; backdrop-filter: blur(10px); }
  .endpoint-terminal { display: inline-grid; width: 1.35rem; height: 1.35rem; place-items: center; border: 1px solid currentColor; border-radius: 50%; }
  ...
  .endpoint:hover:not(:disabled), .endpoint:focus-visible { border-color: var(--courier-color-text); }
  .endpoint.selected { border-color: var(--courier-color-accent); color: var(--courier-color-accent); background: color-mix(in srgb, var(--courier-color-canvas) 84%, transparent); }
  .endpoint:disabled { opacity: 0.28; cursor: not-allowed; }
  .route-readout { height: 10.5rem; }
  .readout-details { display: flex; min-height: 0; align-items: center; gap: 0.6rem; padding: 0.45rem 0.2rem; border-top: 1px solid var(--courier-workbench-line); }
  .flag-list { display: flex; min-width: 0; gap: 0.3rem; overflow: auto; flex-wrap: wrap; }
  .flag { padding: 0.2rem 0.38rem; border: 1px solid var(--courier-workbench-line); border-radius: 999px; color: var(--courier-color-muted); font-family: var(--courier-font-mono); font-size: 0.56rem; white-space: nowrap; }
  .install-interface { display: grid; gap: 1rem; }
  .install-channels { display: flex; gap: 0.35rem; flex-wrap: wrap; }
  .install-channel { appearance: none; display: inline-flex; min-height: 2rem; align-items: center; gap: 0.38rem; padding: 0.28rem 0.62rem; border: 1px solid color-mix(in srgb, var(--courier-color-border) 74%, transparent); border-radius: 999px; color: var(--courier-color-text); background: color-mix(in srgb, var(--courier-color-canvas) 62%, transparent); cursor: pointer; backdrop-filter: blur(10px); }
  .install-channel:hover, .install-channel:focus-visible { border-color: var(--courier-color-text); }
  ...
  @media (max-width: 70rem) { .endpoint-group { grid-template-columns: auto repeat(2, minmax(0, 1fr)); } .endpoint-group > strong { grid-row: 1 / 3; } .cli-workspace { grid-template-columns: minmax(0, 1fr) minmax(17rem, 0.72fr); } }
  @media (max-width: 44rem) {
    ...
    h1 { max-width: 8ch; font-size: clamp(3.4rem, 18vw, 5.5rem); }
